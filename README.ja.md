...
RigorプラグインはこれらのGitHub設定を構成も検証もしません。ローカルcommandの成功表示やモデルの「合格」宣言だけでは不十分です。
...
[MVP limitations](docs/mvp-limitations.md) に意図的な未実装とGitHub Issueを記録しています。0.1.0ではWindows launcher、暗号学的provenance/attestation、意味的なtest品質解析、GitHub上の保護設定検証は保証範囲外です。
