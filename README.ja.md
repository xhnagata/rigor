...
手動Skillの `/rigor:preflight`、`/rigor:contract`、`/rigor:route`、`/rigor:verify`、`/rigor:escalate`、`/rigor:review`、`/rigor:retrospect` が同じCLIフローを案内します。推測によるSkill自動実行が統制を暗黙に成立させないよう、意図的に手動実行にしています。
