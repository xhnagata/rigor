...
`consult-decide` selects the decision deterministically and invokes no model, then persists that decision by default. High/critical risk, low assessment confidence, repeated unchanged failure, security or data-integrity concern, and explicit human request are deterministic triggers. Transmission denial is evaluated before any request and yields Claude-only continuation; unavailable/unknown plugin state produces optional skip, required stop, or Claude-only continuation according to the input policy. Only `request-independent-review` with `invocationAllowed: true` may proceed, and `consult-start` remains the hard transmission prerequisite. Each decision is persisted append-only under `.rigor/evidence/<task>/review-decisions/` with a UUID-suffixed filename; `--dry-run` prints without writing.
...
If verification remains unresolved, the legacy `rigor.escalation-input.v1` still records a narrative handoff unchanged. For bounded automatic decisions, use `rigor.escalation-decision-input.v1` with linked contract, routing plan, attempts, profiles, and optional availability. The pure selector distinguishes retry, adjacent/direct escalation, infrastructure retry/stop, policy/human/budget stops, and no-eligible-candidate stops; saved decisions are append-only under the task's `escalations/` directory. Facts and model-authored speculation remain separate, and no model/provider call is made. See [the orchestration guide](docs/orchestration.md#bounded-escalation-decisions) for evaluation order, migration, CLI usage, and configurable initial-hypothesis thresholds. `rigor retrospect` aggregates redacted local event counts from the gitignored `.rigor/events.jsonl` and, from each task's `outcome.json`, per-candidate success rates (with explicit numerator and denominator), retries, elapsed time, human-intervention minutes, and data-completeness counts. Every rate reports its denominator and every missing-data count so the metrics never hide unavailable measurements; malformed outcome files are tolerated and counted, not fatal. Reported cost is a measurement, and the routing `relativeCost` is an abstract routing weight, never a provider invoice or measured money.

For outcome-based calibration, `rigor eval-report` joins a versioned, split-labelled `rigor.evaluation-manifest.v1` to each task's recorded evidence and reports capability-choice cost using accepted changes as the denominator, with explicit missing-data counts and reviewable over-/under-routing signals; `configuredRelativeCost` is an abstract routing weight, never a price. `rigor eval-replay` shadow-replays the routing selector over recorded plans with a proposed model-profiles file (calibration split only unless `--holdout-final` marks a final evaluation), and `rigor calibration-proposal` records an inert, human-review-required proposal (`status: "proposed"`, `approvalEffect: "none"`) from a distinct `rigor.calibration-proposal-input.v1`, requiring `--manifest` to cross-check every cited evidence task (rejecting unknown tasks and holdout citations unless `holdoutFinalEvaluation` is set) and one or more `--report <file>` arguments whose parsed content and canonical hash back every `evidence.reportHashes`/`replayHash` entry and whose own `manifest.hash` matches `--manifest` (fail-closed on any mismatch), recording the manifest hash and each cited task's split as provenance. Holdout tasks never fit thresholds, no code path applies a proposal, and the CLI never invokes a model. A runnable synthetic sample lives under `test/fixtures/evaluation/`. See [docs/evaluation.md](docs/evaluation.md).

Optionally, `rigor test-integrity-scan` records advisory test-weakening signals and `rigor test-integrity-classify` records unattested human verdicts. `test-integrity-promote`, `test-integrity-replay`, and `test-integrity-waive` add strict version-bound promotion governance, mandatory replay, proportional stop/review/advisory outcomes, single-outcome waivers, and fail-closed rollback. The canonical active registry is `.rigor/test-integrity-active.json`; it is intentionally absent, so **zero signals are promoted today** because the four legacy events have no firings, classifications, outcomes, or version manifests. See [docs/test-integrity.md](docs/test-integrity.md).
...
MVP limitations and planned work are tracked in [MVP limitations](docs/mvp-limitations.md). In particular, Windows launch support, semantic test-quality analysis, and any GitHub-host configuration writes remain out of scope; `rigor governance` verifies host settings read-only but cannot change them. Producer provenance and a consumer-owned verified-install wrapper are implemented ([provenance and verification](docs/provenance.md)), but the ordinary Claude Code marketplace path and any unmanaged install remain outside that guarantee.
