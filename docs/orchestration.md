...
`rigor retrospect` gains a `testIntegrity` section: total events, malformed event and classification counts (never fatal, exactly like `malformedOutcomes`), and per-signal `{ evaluated, fired, unreviewed, humanClassified: { truePositive, falsePositive, uncertain } }`. `evaluated` is the firing-rate denominator and `unreviewed` counts fired signals with no human verdict. False-discovery proportion uses only classified firings (`falsePositive / (truePositive + falsePositive)`), and review coverage is `classified / fired`; there is no false-negative measurement without a labeled audit sample of non-firing events.

Promotion uses `rigor test-integrity-promote --input <json>` to record an inert proposal, `rigor test-integrity-replay --proposal <artifact> [--evidence-root <dir>]` for the required linked shadow comparison, and `rigor test-integrity-waive --input <json>` for a single-outcome, expiring human-reported exception. The canonical active registry is `.rigor/test-integrity-active.json`; absence means no active signals. The shipped registry is absent because the current four unversioned, non-firing events qualify nothing. See [test-integrity.md](test-integrity.md) for the conservative governance floors, version binding, freeze-on-rollback semantics, and trusted-CI anti-bypass boundary.
