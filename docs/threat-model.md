...
| Consultation mutates the worktree despite read-only wording   | planned consultation artifacts compare tree hashes and changed paths before/after; prompt wording alone is not trusted                    |
| External review returns absolute local paths or raw prose     | planned normalization stores repository-relative paths and structured summaries only; raw transcripts are not evidence                    |
