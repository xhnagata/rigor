...
- Status: Implemented. [#25](https://github.com/xhnagata/rigor/issues/25)
  (producer provenance) and [#26](https://github.com/xhnagata/rigor/issues/26)
  (consumer enforcement, via the verified-local-tree / managed-promotion path)
  are both implemented; see the implementation-status note in "#26 —
  independently testable consumer enforcement" below for what changed and what
  remains a documented residual limit.
- Date: 2026-07-11
...
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code marketplace install/update itself        | The documented cache contains the executing bytes, but no attestation-policy callback or required verification command is documented                                                                                                          | Experimentally confirmed absent in Claude Code 2.1.207                              | **Not an enforcement point**; unmanaged marketplace installs remain outside #26's guarantee                                                                                                                                                                                                                                 |
| Organization image/seed promotion pipeline           | Yes, if it downloads an immutable commit/tree, verifies the complete plugin artifact and its `dist/rigor.cjs`, and promotes those same bytes into a read-only Claude Code plugin seed                                                         | Yes                                                                                 | **Recommended managed consumer/verifier**; `scripts/install-verified.mjs` is a reference implementation of this path                                                                                                                                                                                                        |
| Verified local clone used with `claude --plugin-dir` | Yes, if verification and execution use the same immutable local tree and mutation is prevented/detected                                                                                                                                       | Yes for a disciplined wrapper; ordinary manual use is bypassable                    | **Implemented**: `claude --plugin-dir <path>` confirmed to exist in Claude Code 2.1.207; `scripts/install-verified.mjs` verifies, promotes to a read-only seed, and re-checks seed integrity before launch — an enforcement boundary only for a consumer who runs it with independently held policy from outside the plugin |
...
confirms the promoted seed re-packages to the verified digest before launch).
...
experimentally confirmed (Claude Code 2.1.207) to expose no attestation-policy
callback, so an unmanaged personal marketplace install remains outside any
distribution guarantee — this is stated plainly, not hidden. The host-required
CI path protects only the CI verifier consumer; it does not verify the plugin
bytes that hooks, skills, agents, and `bin/rigor` execute, so it does not by
itself close #26 either.
...
**Implementation status (steps 1–6, this task).** Step 1: experimentally
confirmed in Claude Code 2.1.207 that `claude --plugin-dir <path>` exists
(a session-scoped explicit load of a local directory or `.zip`) but the
ordinary marketplace install/update flow exposes no supported pre-activation
...
that the promoted seed re-packages to the exact verified digest (closing the
verify/execute TOCTOU gap) before instructing/launching
...
(only an already-verified pinned version within a consumer max-age policy);
anything else refuses. Step 5: [provenance.md](../provenance.md#consumer-enforcement-26)
documents migration, rollback by pinned commit/digest, and break-glass. Step 6:
stated plainly in provenance.md's bypassability section and here — an
unmanaged marketplace user is outside this guarantee.

#26 remains blocked **for the ordinary marketplace path**: no supported
mechanism there can verify the exact plugin bytes before hooks, skills, agents,
or `bin/rigor` execute. It is implemented for a consumer who runs
`scripts/install-verified.mjs` independently: verification and execution
share the same immutable local tree (the seed), and the consumer holds
signer/source/deny expectations in its own policy file, separately from the
Rigor release. #26 would remain blocked for that consumer too if verification
fetched one object but Claude Code executed a separately fetched copy, or if
the consumer could not hold its policy independently of the producer — neither
holds for the implemented wrapper when used as documented.
