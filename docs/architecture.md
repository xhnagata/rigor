...
The CLI reads `.rigor/policy.json`, an intent JSON file, and Git facts. Pure policy evaluation produces a preflight artifact. Contract creation consumes that artifact and explicit acceptance criteria. Verification rechecks scope and executes configured commands directly (no shell), storing only status, duration, a digest, and normalized per-check failure facts (a category, error class, normalized failed-test names, and opaque digests) with a verification-wide failure fingerprint; raw command output is never persisted. Escalation and review consume the linked artifacts. Retrospective reads redacted JSONL events and aggregates test-integrity shadow evidence. Test-integrity proposals are inert, version-bound records; replay is mandatory before a separately protected `.rigor/test-integrity-active.json` registry entry can evaluate active. The registry is absent by default, so current behavior remains shadow-only. CI compares pinned base/head protected subjects and escalates same-diff registry/configuration/verifier changes to human review. This is a deterministic consistency invariant under trusted CI and protected Git history, not identity or provenance authentication. See [test-integrity.md](test-integrity.md). A separate advisory routing evaluation (`rigor eval-report`/`eval-replay`/`calibration-proposal`) remains isolated from these controls.
