...
The CLI reads `.rigor/policy.json`, an intent JSON file, and Git facts. Pure policy evaluation produces a preflight artifact. Contract creation consumes that artifact and explicit acceptance criteria. Verification rechecks scope and executes configured commands directly (no shell), storing only status, duration, a digest, and normalized per-check failure facts (a category, error class, normalized failed-test names, and opaque digests) with a verification-wide failure fingerprint; raw command output is never persisted. Escalation and review consume the linked artifacts. Retrospective reads redacted JSONL events and aggregates any test-integrity shadow evidence. A separate, advisory evaluation (`rigor eval-report`/`eval-replay`/`calibration-proposal`) joins a versioned evaluation manifest to recorded evidence to compare capability choices by accepted-change cost, shadow-replays proposed routing changes on the calibration split only, and records inert human-review-required proposals; it invokes no model and hard-separates holdout tasks from any threshold fitting. See the [evaluation guide](evaluation.md). A separate, advisory `rigor test-integrity-scan` records deterministic test-weakening signals over a base/head (or worktree) diff as record-only `rigor.test-integrity-event.v1` evidence; it never reads or affects the verification, attempt, review, or CI paths.
