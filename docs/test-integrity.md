...
3. **Shadow candidate (#22, implemented)** — recorded as a
   ...
   fired-event exists for them today.
4. **Conditional shadow** — shadow collection only where the repository
   already runs the required tool as a configured check with comparable results;
   never a Rigor default. TI-11 (coverage) and TI-12 (mutation score) are on
   this rung; neither is implemented or scanned today.
5. **Gate (implemented)** — deterministic enforcement that exists today.
   ...
   this rung, and this catalog adds nothing to it. The implemented #13 gate
   (TI-02/TI-03/TI-04) constrains cross-attempt `progress` classification
   (`reduced`/`incomparable`) only; it never changes verification pass/fail,
   review, or merge outcomes.
...
| A4  | Relax assertions or widen tolerances so failing tests pass                                 | Nothing: totals and checks are unchanged                                                                                                                            | TI-07 (net assertion-token decline) partially; TI-13/TI-15 are proposed lower-priority shadow candidates, not yet collected, and their weakening interpretation stays advisory — the implemented mechanism is blind here |
...
advisory. Exact matcher-strength comparison (TI-15) uses a configured
per-framework substitution-pair table: that a configured weakening pair (for
example `toEqual` → `toBeDefined`) matched is deterministic, but that the
substitution indicates weakened test integrity stays advisory. TI-15 is a
proposed addition to shadow collection at lower priority than the implemented
TI-05..TI-09 set; it is not yet evaluated by the implemented scanner. AST-level
assertion counting (TI-10) requires a per-language parser and is tool-derived.
...
widened tolerance is justified is a semantic question. TI-13 has a
deterministically computable trigger for an explicitly configured matcher list:
that a configured matcher argument widened is deterministic, but that the
widening weakens test integrity stays advisory. TI-13 is a proposed addition to
shadow collection at lower priority than the implemented TI-05..TI-09 set; it is
not yet evaluated by the implemented scanner.
...
configuration. TI-12 is a conditional shadow signal, restricted to
repositories that already run a pinned mutation tool as a configured check and
that produce scores comparable over the same configuration and mutant set;
Rigor never introduces a default mutation run. Like TI-11 coverage, it is not
implemented and not scanned today — its value would depend entirely on
repository-supplied, pinned, comparable runs the repository already performs.
...
| --------------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ----------------------------------- |
| TI-01 configured-check definition changed in `.rigor/policy.json`     | deterministic fact       | high — the verification surface itself changed                                                                                                             | none                                                                   | low for detecting a definition change; no weakening intent inferred | base policy loaded independently by CI                                                | gate (implemented)                  |
...
| TI-11 coverage percentage decline                                     | tool-derived measurement | high that the reported percentage declined under identical tool, version, configuration, and scope; medium (advisory) that the decline indicates weakening | high — tool- and configuration-specific                                | medium (moved lines, changed scope, tool nondeterminism)            | repository-configured coverage tool, baseline, and threshold                          | conditional shadow                  |
| TI-12 mutation-score decline                                          | tool-derived measurement | medium given a pinned tool, configuration, mutant set, and comparable completed runs                                                                       | high                                                                   | high                                                                | repository-configured pinned mutation check with a fixed configuration and mutant set | conditional shadow                  |
| TI-13 tolerance-matcher argument widened                              | advisory interpretation  | high that the configured matcher argument widened (deterministic); low–medium that it indicates weakened test integrity (advisory)                         | high — per-matcher direction semantics                                 | medium (justified re-tuning)                                        | configured matcher list with per-matcher direction table                              | shadow candidate (proposed)         |
...
| TI-15 assertion matcher replaced by a weaker one                      | advisory interpretation  | high that the configured weakening pair matched (deterministic); low–medium that it indicates weakened test integrity (advisory)                           | high — per-framework substitution-pair table                           | medium (legitimate matcher changes)                                 | configured matcher-strength pairs                                                     | shadow candidate (proposed)         |
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TI-01  | A PR removes or edits the `git-diff-check` entry in the `.rigor/policy.json` `checks` array                                                                                                                                           | An unrelated `.rigor/policy.json` edit that leaves the `checks` array untouched (e.g. editing a rule's `reason` string)                                        |
...
| TI-08  | A diff regenerates thirty `*.snap` files alongside `src/**` changes (the event records counts, not a verdict)                                                                                                                         | A diff that changes no path matching the snapshot conventions; or a snapshot-only diff with no accompanying implementation change (the trigger is conjunctive) |
...
| TI-12  | The repository-configured pinned mutation check reports a score decline over identical configuration and mutant set across two comparable completed runs                                                                              | No mutation check is configured, or the tool, configuration, or mutant set changed between runs                                                                |
...
are TI-02/TI-03/TI-04 in this catalog. This implemented gate constrains
cross-attempt `progress` classification (`reduced`/`incomparable`) only: it
never changes verification pass/fail, review, or merge outcomes.
...
The five highest-value candidates below are now **implemented** shadow
collection (#22, via #45); the rest of this section records the remaining
proposed, conditional, advisory, and rejected dispositions. In descending
expected value, the implemented set is:
...
Proposed additions to shadow collection, at lower priority than the five above
and **not yet evaluated by the implemented scanner**: **TI-13** tolerance-matcher
argument widened and **TI-15** assertion matcher replaced by a weaker one — each
has a deterministic rule-match trigger whose weakening interpretation stays
advisory; no event fires for them today. Conditional shadow, only where the
repository already runs the required tool as a configured check with comparable
results and never as a Rigor default (neither implemented nor collected today):
**TI-11** coverage decline and **TI-12** mutation-score decline. Advisory only,
not collected: TI-10, TI-14. Rejected: TI-16 (LLM verdict as an authoritative
check).
